...
// rules in file order. Unparseable lines are silently skipped — there is no
// official "soft fail" representation in this parser; the running server's
// `pg_hba_file_rules` view (exposed by the postgres provider) carries the
// canonical parse-error data.
func ParseHba(content string) []HbaRule {
	...
	for i, raw := range strings.Split(content, "\n") {
		...
		tokens := tokenizeHba(line)
		if len(tokens) < 4 {
			continue
		}
		rule := HbaRule{LineNumber: i + 1, Type: tokens[0]}
		switch tokens[0] {
		case "local":
...
// of mappings in file order. Lines that don't have at least three tokens
...
func ParseIdent(content string) []IdentMapping {
	...
	for i, raw := range strings.Split(content, "\n") {
		...
		tokens := tokenizeHba(line) // same quoting rules
		if len(tokens) < 3 {
			continue
		}
