...
// Identifier returns the first accessible company's UUID. Gusto tokens are
// scoped to one or more companies, so this is stable for single-company
// tokens (the common case) and deterministic for multi-company tokens
// because Gusto returns companies in insertion order.
